...
$$
\begin{aligned}
U'(z) & = \mathbb{E}[(\tilde{a} - (1 + r_f)p) u'(y_0 (1 + r_f))] \\
& + z \mathbb{E}[(\tilde{a} - (1 + r_f)p)^2 u''(y_0 (1 + r_f))] + o(z)
\end{aligned} 
$$

The first order condition is only approximately zero at $z=0$ if the first term is zero, i.e.
$$\mathbb{E}[(\tilde{a} - (1 + r_f)p)] = 0 \Leftrightarrow \frac{\mathbb{E}[\tilde{a}]}{1+r_f} = p$$
...
For additional practice problems (with separate solutions), see:

➡️ [Exercises: Decision Making under Uncertainty](exercises.qmd)
